...
и выбирает до трёх российских и трёх контрольных зарубежных узлов. Через них
выполняются вспомогательный ping, TCP-проверка порта VLESS и UDP-проверка порта
Hysteria2. Результат UDP обозначается как `OPEN_OR_FILTERED` и не считается
подтверждением работоспособности протокола.
